...
projectChatRouter.post("/", requireAuth, async (req, res) => {
    ...
    const {
        ...
        req.body as {
            ...
            save_generated_documents?: boolean;
        };
    const generatedProjectId =
        save_generated_documents === false ? null : projectId;
    ...
    if (
        ...
        try {
            ...
            console.error("[project-chat/counselbridge] error:", err);
            try {
                write(
                    `data: ${JSON.stringify({ type: "error", message: "CounselBridge error" })}\n\n`,
                );
                write("data: [DONE]\n\n");
            } catch {
                /* ignore */
