...
userRouter.post("/profile", requireAuth, async (_req, res) => {
  ...
  if (error) {
    console.error("[user/profile] ensure failed", error);
    return void res.status(500).json({ detail: "Unable to update profile" });
  }
  res.json({ ok: true });
});
...
userRouter.get("/profile", requireAuth, async (_req, res) => {
  ...
  if (error) {
    ...
    return void res.status(500).json({ detail: "Unable to load profile" });
  }
...
userRouter.patch("/profile", requireAuth, async (req, res) => {
  ...
  const ensureError = await ensureProfileRow(db, userId);
  if (ensureError) {
    console.error("[user/profile] ensure before patch failed", ensureError);
    return void res.status(500).json({ detail: "Unable to update profile" });
  }
  ...
  const { error: updateError } = await db
    ...
    .eq("user_id", userId);
  if (updateError) {
    console.error("[user/profile] patch failed", updateError);
    return void res.status(500).json({ detail: "Unable to update profile" });
  }

  const { data, error } = await loadProfile(db, userId);
  if (error) {
    console.error("[user/profile] reload after patch failed", error);
    return void res.status(500).json({ detail: "Unable to load profile" });
  }
  const apiKeyStatus = await getUserApiKeyStatus(userId, db);
  res.json({ ...data, apiKeyStatus });
...
userRouter.delete("/account", requireAuth, async (req, res) => {
  if (req.body?.confirmation !== "DELETE") {
    return void res.status(400).json({
      detail: 'Account deletion requires confirmation: "DELETE"',
    });
  }
  ...
  if (error) {
    console.error("[user/account] delete failed", error);
    return void res.status(500).json({ detail: "Unable to delete account" });
  }
  res.status(204).send();
});
