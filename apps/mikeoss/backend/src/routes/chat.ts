...
import {
    counselBridgeEnabled,
    isCounselBridgeWorkflowId,
    runCounselBridgeProjectTurn,
    shouldRouteToCounselBridge,
} from "../lib/counselbridge";
...
function streamErrorMessage(error: unknown, _model: string | undefined): string {
...
chatRouter.get("/", requireAuth, async (req, res) => {
    ...
    const { data: ownProjects, error: projErr } = await db
        ...
        .eq("user_id", userId);
    if (projErr) {
        console.error("[chat] failed to list owned project ids", projErr);
        return void res.status(500).json({ detail: "Unable to list chats" });
    }
...
chatRouter.post("/create", requireAuth, async (req, res) => {
    ...
    if (error) {
        ...
        return void res.status(500).json({ detail: "Unable to create chat" });
    }
    res.json({ id: data.id });
});
...
chatRouter.delete("/:chatId", requireAuth, async (req, res) => {
    ...
    if (error) {
        ...
        return void res.status(500).json({ detail: "Unable to delete chat" });
    }
    res.status(204).send();
});
...
chatRouter.post("/", requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (
            lastUser?.content &&
            counselBridgeEnabled() &&
            (shouldRouteToCounselBridge(lastUser.content) ||
                isCounselBridgeWorkflowId(lastUser.workflow?.id))
        ) {
            write(
                `data: ${JSON.stringify({ type: "tool_call_start", name: "counselbridge_review" })}\n\n`,
            );
            const result = await runCounselBridgeProjectTurn({
                ...
                docStore,
                docIndex,
                attachedDocuments: lastUser.files
                    ?.filter((file) => file.document_id)
                    .map((file) => ({
                        filename: file.filename,
                        document_id: file.document_id as string,
                    })),
                apiKeys,
            });

            const eventsForDb: unknown[] = [];
            for (const event of result.events) {
                if (event.type === "doc_created") {
                    write(
                        `data: ${JSON.stringify({ type: "doc_created_start", filename: event.filename })}\n\n`,
                    );
                    write(`data: ${JSON.stringify(event)}\n\n`);
                    eventsForDb.push(event);
                } else if (event.type === "content") {
                    write(
                        `data: ${JSON.stringify({ type: "content_delta", text: event.text })}\n\n`,
                    );
                    write(
                        `data: ${JSON.stringify({ type: "content_done" })}\n\n`,
                    );
                    eventsForDb.push(event);
                }
            }
            ...
            await db.from("chat_messages").insert({
                chat_id: chatId,
                role: "assistant",
                content: eventsForDb.length ? eventsForDb : null,
                annotations: null,
            });
        ...
        try {
            write(
                `data: ${JSON.stringify({ type: "error", message: streamErrorMessage(err, model) })}\n\n`,
            );
            write("data: [DONE]\n\n");
        } catch {
            /* ignore */
