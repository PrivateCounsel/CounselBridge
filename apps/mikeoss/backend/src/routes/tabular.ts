...
function serverError(
    ...
    console.error(`[tabular] ${context}`, error);
...
tabularRouter.get("/", requireAuth, async (req, res) => {
    ...
    if (ownErr)
        return void serverError(res, "list own reviews failed", ownErr, "Unable to list tabular reviews");
        ...
        if (Array.isArray(req.body.document_ids)) {
            ...
            if (removedDocIds.length > 0) {
                ...
                if (deleteError)
                    return void serverError(
                        res,
                        "delete removed document cells failed",
                        deleteError,
                        "Unable to update review cells",
                    );
            }
...
tabularRouter.delete("/:reviewId", requireAuth, async (req, res) => {
    ...
    const { error } = await db
        ...
        .eq("user_id", userId);
    if (error)
        return void serverError(res, "delete review failed", error, "Unable to delete review");
    res.status(204).send();
});
...
tabularRouter.post("/:reviewId/clear-cells", requireAuth, async (req, res) => {
    ...
    if (error)
        return void serverError(res, "clear cells failed", error, "Unable to clear cells");
    res.status(204).send();
});
...
tabularRouter.delete(
    ...
    async (req, res) => {
        ...
        const { error } = await db
            ...
            .eq("user_id", userId);
        if (error)
            return void serverError(res, "delete chat failed", error, "Unable to delete chat");
        res.status(204).send();
    },
);
