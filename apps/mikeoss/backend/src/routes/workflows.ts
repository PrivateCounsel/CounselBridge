...
function serverError(
  ...
  console.error(`[workflows] ${context}`, error);
...
workflowsRouter.get("/", requireAuth, async (req, res) => {
  ...
  if (ownErr) return void serverError(res, "list own workflows failed", ownErr, "Unable to list workflows");
...
workflowsRouter.post("/", requireAuth, async (req, res) => {
  ...
  const { data, error } = await db
    ...
    .single();
  if (error)
    return void serverError(res, "create failed", error, "Unable to create workflow");
...
workflowsRouter.delete("/:workflowId", requireAuth, async (req, res) => {
  ...
  const { error } = await db
    ...
    .eq("is_system", false);
  if (error)
    return void serverError(res, "delete failed", error, "Unable to delete workflow");
  res.status(204).send();
});
...
workflowsRouter.get("/hidden", requireAuth, async (req, res) => {
  ...
  const { data, error } = await db
    ...
    .eq("user_id", userId);
  if (error)
    return void serverError(res, "list hidden failed", error, "Unable to list hidden workflows");
...
workflowsRouter.post("/hidden", requireAuth, async (req, res) => {
  ...
  if (error)
    return void serverError(res, "hide workflow failed", error, "Unable to hide workflow");
  res.status(204).send();
});
...
workflowsRouter.delete("/hidden/:workflowId", requireAuth, async (req, res) => {
  ...
  const { error } = await db
    ...
    .eq("workflow_id", workflowId);
  if (error)
    return void serverError(res, "unhide workflow failed", error, "Unable to unhide workflow");
  res.status(204).send();
});
...
workflowsRouter.get("/:workflowId/shares", requireAuth, async (req, res) => {
  ...
  if (error)
    return void serverError(res, "list shares failed", error, "Unable to list workflow shares");
...
workflowsRouter.post("/:workflowId/share", requireAuth, async (req, res) => {
  ...
  if (error)
    return void serverError(res, "share failed", error, "Unable to share workflow");
