...
projectsRouter.get("/", requireAuth, async (req, res) => {
  ...
  if (ownError) {
    console.error("[projects] list owned projects failed", ownError);
    return void res.status(500).json({ detail: "Unable to list projects" });
  }
  ...
  const { data: sharedProjects, error: sharedError } = userEmail
    ...
    : { data: [], error: null };
  if (sharedError) {
    console.error("[projects] list shared projects failed", sharedError);
    return void res.status(500).json({ detail: "Unable to list projects" });
  }
...
projectsRouter.post("/", requireAuth, async (req, res) => {
  ...
  if (error) {
    ...
    return void res.status(500).json({ detail: "Unable to create project" });
  }
...
projectsRouter.delete("/:projectId", requireAuth, async (req, res) => {
  ...
  if (error) {
    ...
    return void res.status(500).json({ detail: "Unable to delete project" });
  }
  res.status(204).send();
});
...
projectsRouter.get("/:projectId/chats", requireAuth, async (req, res) => {
  ...
  const { data, error } = await db
    ...
    .order("created_at", { ascending: false });
  if (error) {
    console.error("[projects] list chats failed", error);
    return void res.status(500).json({ detail: "Unable to list project chats" });
  }
  res.json(data ?? []);
});
...
projectsRouter.post("/:projectId/folders", requireAuth, async (req, res) => {
  ...
  if (error) {
    console.error("[projects] create folder failed", error);
    return void res.status(500).json({ detail: "Unable to create folder" });
  }
...
projectsRouter.delete("/:projectId/folders/:folderId", requireAuth, async (req, res) => {
  ...
  if (error) {
    console.error("[projects] delete folder failed", error);
    return void res.status(500).json({ detail: "Unable to delete folder" });
  }
  res.status(204).send();
});
...
export async function handleDocumentUpload(
  ...
  try {
    ...
    console.error("[projects] document processing failed", e);
