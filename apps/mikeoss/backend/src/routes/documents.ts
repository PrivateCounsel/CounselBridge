...
documentsRouter.get("/", requireAuth, async (req, res) => {
  ...
  if (error) {
    ...
    return void res.status(500).json({ detail: "Unable to list documents" });
  }
...
documentsRouter.post("/download-zip", requireAuth, async (req, res) => {
  ...
  if (error) {
    ...
    return void res.status(500).json({ detail: "Unable to load documents" });
  }
...
documentsRouter.post(
  ...
  async (req, res) => {
    ...
    const validationError = validateDocumentUpload(file, suffix);
    if (validationError)
    ...
    if (verErr || !versionRow) {
      ...
      if (pdfStoragePath && pdfStoragePath !== key) await deleteFile(pdfStoragePath);
      return void res
        .status(500)
...
async function handleEditResolution(
  ...
  const debugEditResolution = (message: string, data?: unknown) => {
    if (!isDev) return;
  ...
  debugEditResolution(`[edit-resolution] incoming ${mode}`, {
    userId,
    documentId,
    editId,
  });
  ...
  const { data: edit, error: editErr } = await db
    ...
    .eq("document_id", documentId)
    .single();
  debugEditResolution(`[edit-resolution] fetched edit row`, { edit, editErr });
  if (!edit) {
    debugEditResolution(`[edit-resolution] edit not found, returning 404`);
  ...
  if (edit.status !== "pending") {
    debugEditResolution(`[edit-resolution] edit already resolved`, {
      editId,
      status: edit.status,
    });
    const { data: doc } = await db
      ...
      .single();
    if (!doc) {
      debugEditResolution(`[edit-resolution] doc not found for resolved edit`);
      return void res.status(404).json({ detail: "Document not found" });
    }
    const accessResolved = await ensureDocAccess(doc, userId, userEmail, db);
    if (!accessResolved.ok) {
      debugEditResolution(`[edit-resolution] doc access denied for resolved edit`);
      return void res.status(404).json({ detail: "Document not found" });
    }
    ...
    debugEditResolution(`[edit-resolution] returning already-resolved payload`, payload);
  ...
  const { data: doc, error: docErr } = await db
    ...
    .eq("id", documentId)
    .single();
  debugEditResolution(`[edit-resolution] fetched doc`, { doc, docErr });
  if (!doc)
    return void res.status(404).json({ detail: "Document not found" });
  const access = await ensureDocAccess(doc, userId, userEmail, db);
  if (!access.ok)
  ...
  debugEditResolution(`[edit-resolution] resolved latestPath`, {
    latestPath,
    current_version_id: doc.current_version_id,
  });
  if (!latestPath)
  ...
  debugEditResolution(`[edit-resolution] downloaded bytes`, {
  ...
  debugEditResolution(`[edit-resolution] resolveTrackedChange result`, {
    mode,
  ...
  if (!found) {
    ...
    const { error: updErr } = await db
      ...
      .eq("id", editId);
    debugEditResolution(`[edit-resolution] status-only update`, { updErr });
    ...
    debugEditResolution(`[edit-resolution] returning not-found payload`, payload);
    return void res.status(200).json(payload);
  }
  ...
  debugEditResolution(`[edit-resolution] overwriting bytes in place`, {
    latestPath,
  ...
  const { error: statusErr } = await db
    ...
    .eq("id", editId);
  debugEditResolution(`[edit-resolution] updated document_edits status`, {
    editId,
  ...
  const { count: remainingPending } = await db
    ...
    .eq("document_id", documentId)
    .eq("status", "pending");
  debugEditResolution(`[edit-resolution] remaining pending count`, { remainingPending });
  ...
  debugEditResolution(`[edit-resolution] returning success payload`, payload);
  res.json(payload);
}
...
async function handleDocumentUpload(
  ...
  try {
    ...
    console.error("[documents] document processing failed", e);
    await db.from("documents").update({ status: "error" }).eq("id", doc.id);
    return void res
      .status(500)
      .json({ detail: "Document processing failed" });
  }
}
