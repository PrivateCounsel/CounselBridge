 ...
 * Local Supabase Storage, MinIO, and Cloudflare R2 all work through the AWS SDK.
 *
 * Required env vars:
...
if (storageEnabled && !process.env.R2_BUCKET_NAME) {
  throw new Error("R2_BUCKET_NAME is required when S3-compatible storage is enabled");
}
