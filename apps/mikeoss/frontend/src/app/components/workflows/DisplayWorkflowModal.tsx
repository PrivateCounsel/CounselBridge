...
import { createChat, createTabularReview } from "@/app/lib/mikeApi";
...
export function DisplayWorkflowModal({ workflows, workflow, onClose }: Props) {
    ...
    const { setNewChatMessages } = useChatHistoryContext();
    const {
        loading: dirLoading,
        projects,
        standaloneDocuments,
    ...
    const workflowPageHref = `/workflows/${wf.id}?from=${encodeURIComponent(pathname)}`;
    ...
    async function handleStartChat() {
        if (saving) return;
        setStartError(null);
        if (inProject && !selectedProjectId) {
            setStartError("Select a project, or turn off Create in a project.");
            return;
        }
        setSaving(true);
        try {
            ...
            const allDocs: MikeDocument[] = inProject
            ...
            console.error("Failed to start workflow chat", err);
            setStartError("Could not start chat. Please try again.");
        } finally {
            setSaving(false);
        }
    }
            ...
            >
                ...
                {screen === "configure" && wf.type === "assistant" && (
                    <>
                        ...
                        <div className="border-t border-gray-200 px-5 py-3 flex items-center justify-between shrink-0">
                            <span className="min-w-0 pr-3 text-xs text-gray-400">
                                {startError
                                    ? startError
