...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput(
    ...
    const [saveGeneratedDocuments, setSaveGeneratedDocuments] = useState(true);
    ...
    const handleSubmit = () => {
        ...
        if ((!query && !selectedWorkflow) || isLoading) return;
        ...
        onSubmit?.({
            role: "user",
            content: query || "implement workflow",
            files: files.length > 0 ? files : undefined,
            workflow: wf ?? undefined,
            model,
            save_generated_documents: projectName
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            {projectName && (
                                <button
                                    type="button"
                                    onClick={() =>
                                        setSaveGeneratedDocuments((v) => !v)
                                    }
                                    title="Save generated Word documents into this project's Documents tab"
                                    className={`hidden sm:inline-flex items-center gap-1 rounded-lg border px-2 h-8 text-xs transition-colors ${
                                        saveGeneratedDocuments
