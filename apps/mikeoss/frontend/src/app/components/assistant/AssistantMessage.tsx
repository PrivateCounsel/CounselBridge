...
import { getApiBaseUrl, getDocumentUrl } from "@/app/lib/mikeApi";
...
function DocDownloadBlock({
    ...
    const fetchDownload = async (url: string, token?: string | null) => {
        const isBackendUrl = url.startsWith(API_BASE) || url.startsWith("/");
        ...
        if (!resp.ok) {
            ...
            try {
                ...
                // Non-JSON download errors are reported with the HTTP status.
            }
            throw new Error(detail || `Download failed (HTTP ${resp.status})`);
        }
        return resp;
    };

    const saveBlob = (blob: Blob, suggestedFilename: string) => {
    ...
    const openDownloadUrl = (url: string, suggestedFilename: string) => {
        const a = document.createElement("a");
        a.href = url;
        a.download = suggestedFilename;
        document.body.appendChild(a);
    ...
    const handleDownload = async (e?: {
        ...
        try {
            ...
            let resp: Response;
            try {
                resp = await fetchDownload(href, token);
            } catch (err) {
                if (!documentId) throw err;
                const refreshed = await getDocumentUrl(
                    documentId,
                    versionId ?? null,
                );
                openDownloadUrl(refreshed.url, refreshed.filename || filename);
                return;
            }
            const blob = await resp.blob();
            saveBlob(blob, filename);
        } catch (err) {
            console.error("[DocDownloadBlock] download failed", err);
            setDownloadError(
                err instanceof Error
                    ? err.message
                    : "Download failed. Open the document or regenerate it.",
            );
        } finally {
            setBusy(false);
        }
